...
function ProductList({ onHomeClick }) {
  ...
  // Cantidad total en el carrito (badge)
  const cartCount = useSelector((state) =>
    state.cart.items.reduce((sum, it) => sum + it.quantity, 0)
  );

  // Añadir al carrito (normaliza costo y marca botón Added)
  const handleAddToCart = (product) => {
    const normalized = {
      ...product,
      cost:
        typeof product.cost === "string"
          ? Number(product.cost.replace(/[^0-9.]/g, ""))
          : Number(product.cost),
    };
  ...
  const plantsArray = [
    ...
    },
    {
      ...
      plants: [
        ...
        { name: "Hyacinth", image: "https://cdn.pixabay.com/photo/2019/04/07/20/20/hyacinth-4110726_1280.jpg", description: "Hyacinth is a beautiful flowering plant known for its fragrance.", cost: "$22" },
      ],
    },
  ];

  // Estilos inline de la navbar
  const styleObj = {
        ...
        <div style={styleObjUl}>
          ...
          <div style={{ position: "relative" }}>
            <a href="#" onClick={handleCartClick} style={styleA}>
              <h1 className="cart" aria-label="Open cart">
        ...
        <div className="product-grid">
          <div className="product-list">
            {plantsArray.map((category, index) => (
              <div key={category.category || index} style={{ gridColumn: "1 / -1" }}>
                <h1 style={{ margin: "16px 0" }}>{category.category}</h1>
